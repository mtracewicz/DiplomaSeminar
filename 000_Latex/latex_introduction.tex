...
\begin{figure}[h!]
    ...
    \label{fig:coffee}
\end{figure}
Figure environment will take care of numberinga and positioning of image. To include image user needs to
...
\textbackslash caption allows user to add visable caption to an image, \textbackslash label adds invisible label
which can be helpfull if user wants to refer to this image later on.
User can sugest or force how \LaTeX will position an image. It is done by putting one of following values into squere
...
allows to do this preaty easly by using "subcaption" package.
\begin{verbatim}
\begin{figure}[h!]
    \centering
    \begin{subfigure}[b]{0.4\linewidth}
...
To automate createion of bibliography user needs to use external tools like:
...
First comand is used to tell BibTex which file ".bib" file should it use and second one is
responsible for seting style in which our bibliography will be displayed. ".bib" files can easly be created
...
If we add BibTex we need to change our compilation proces to:
\begin{lstlisting}
    pdflatex [source file path]
...
is used as new row. Vertical lines are passed in squere brackets to "tabular" environment as "|" character.
The letters which are next to them tell \LaTeX how to allign content in column:
\begin{itemize}
    \item l - left
    \item c - center
    \item r - right
\end{itemize}
Row separators can be added with \textbackslash hline. To align numbers according to deciaml point use
...
\end{center}
\end{table}
Those two commands can be combained like this:
\begin{verbatim}
\begin{table}[h!]
\begin{center}
