...
def create_model(input_img,n_filters=16, dropout=0.5, batchnorm=True):
    # contracting path
    c1 = tf.keras.conv2d_block(input_img, n_filters=n_filters*1, kernel_size=3, batchnorm=batchnorm)
    p1 = tf.keras.MaxPooling2D((2, 2)) (c1)
    p1 = tf.keras.Dropout(tf.keras.dropout*0.5)(p1)

    c2 = tf.keras.conv2d_block(p1, n_filters=n_filters*2, kernel_size=3, batchnorm=batchnorm)
    p2 = tf.keras.MaxPooling2D((2, 2)) (c2)
    p2 = tf.keras.Dropout(tf.keras.dropout)(p2)

    c3 = tf.keras.conv2d_block(p2, n_filters=n_filters*4, kernel_size=3, batchnorm=batchnorm)
    p3 = tf.keras.MaxPooling2D((2, 2)) (c3)
    p3 = tf.keras.Dropout(tf.keras.dropout)(p3)

    c4 = tf.keras.conv2d_block(p3, n_filters=n_filters*8, kernel_size=3, batchnorm=batchnorm)
    p4 = tf.keras.MaxPooling2D(pool_size=(2, 2)) (c4)
    p4 = tf.keras.Dropout(tf.keras.dropout)(p4)
    
    c5 = tf.keras.conv2d_block(p4, n_filters=n_filters*16, kernel_size=3, batchnorm=batchnorm)
    ...
    c6 = tf.keras.conv2d_block(u6, n_filters=n_filters*8, kernel_size=3, batchnorm=batchnorm)
    ...
    c7 = tf.keras.conv2d_block(u7, n_filters=n_filters*4, kernel_size=3, batchnorm=batchnorm)
    ...
    c8 = tf.keras.conv2d_block(u8, n_filters=n_filters*2, kernel_size=3, batchnorm=batchnorm)
    ...
    c9 = tf.keras.conv2d_block(u9, n_filters=n_filters*1, kernel_size=3, batchnorm=batchnorm)
