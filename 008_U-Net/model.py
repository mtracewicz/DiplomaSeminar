...
class myModel(tf.keras.Model):
    def call(self, inputs):
        ...
        # u7 = tf.keras.layers.Conv2DTranspose(n_filters*4, (3, 3), strides=(2, 2), padding='same') (c5)
        # u7 = tf.keras.layers.concatenate([u7, c3])
        # u7 = tf.keras.layers.Dropout(dropout)(u7)

        # c7 = conv2d_block(u7, n_filters=n_filters*4, kernel_size=3, batchnorm=batchnorm)
        # u8 = tf.keras.layers.Conv2DTranspose(n_filters*2, (3, 3), strides=(2, 2), padding='same') (c5)
        # u8 = tf.keras.layers.concatenate([u8, c2])
        # u8 = tf.keras.layers.Dropout(dropout)(u8)
        # c8 = conv2d_block(u8, n_filters=n_filters*2, kernel_size=3, batchnorm=batchnorm)

        u9 = tf.keras.layers.Conv2DTranspose(n_filters*1, (3, 3), strides=(2, 2), padding='same') (c5)
...
def create_model2(input_img,n_filters=2, dropout=0.5, batchnorm=True):
