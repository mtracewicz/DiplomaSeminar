...
if __name__ == "__main__":
    ...
    #Loading data
    IMAGES_DIRECTORY= os.getcwd()+'/008_U-Net/_in'
    filenames = os.listdir(IMAGES_DIRECTORY)
    number_of_images = len(filenames)
    x_train = np.ones((number_of_images,1200,1600,4))
    for i,filename in enumerate(filenames):
        img = Image.open(f"{IMAGES_DIRECTORY}/{filename}")
        x_train[i] = np.array(img)

    IMAGES_DIRECTORY= os.getcwd()+'/008_U-Net/_out'
    filenames = os.listdir(IMAGES_DIRECTORY)
    number_of_images = len(filenames)
    y_train = np.ones((number_of_images,1200,1600,4))
    for i,filename in enumerate(filenames):
        img = Image.open(f"{IMAGES_DIRECTORY}/{filename}")
    ...
    x_train_normalized = (x_train/255.0).reshape(x_train.shape[0], 1200, 1600, 4)
    # Prepare grand truth
    y_train_normalized = (y_train/255.0).reshape(y_train.shape[0], 1200, 1600, 4))
    ...
    res = Image.fromarray(((model.predict(np.array(Image.open(f'{os.getcwd()}/008_U-Net/)_in/0.png'))*255))).astype(np.uint8))
    res.save("res.jpeg")
    print('Finished!')
